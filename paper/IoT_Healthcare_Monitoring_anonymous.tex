...
\author*[1]{\fnm{Anonymous} \sur{Author(s)}}\email{Email omitted for double-blind review}

\affil*[1]{\orgname{Affiliation omitted for double-blind review}}
...
\begin{enumerate}
  ...
  \item An MQTT/ThingsBoard cloud-integration layer with a defined
    ...
    (Sect.~\ref{sec:mqtt}).
  \item A working Android companion application with real-time
    vital-sign cards, threshold-based alerting, and MQTT integration
    (Sect.~\ref{sec:mobile}).
\end{enumerate}
...
microcontroller is an ESP32-WROOM-32D development board~\cite{espressif}
(dual-core Xtensa LX6, integrated Wi-Fi/BLE), replacing the Arduino
ATmega328 and its external wireless module. Ambient
temperature\footnote{Environmental, not core-body, temperature;
tracked as a heat-stress indicator given obese adults' reduced
heat-dissipation capacity.} is sampled by a DS18B20 waterproof
digital probe~\cite{ds18b20ds} over a 1-Wire bus, removing the ADC
quantisation noise of the baseline's analogue LM35 chain.
SpO\textsubscript{2} and heart rate are sampled
by a MAX30102 pulse-oximeter module~\cite{max30102ds} over
...
MQTT's fixed binary header introduces significantly less protocol
overhead than HTTP's textual header structure, and its
publish/subscribe model eliminates the polling round-trip required
by request-based protocols~\cite{mqtt311}. Quality-of-Service level 1
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.26\linewidth]{images/fig-android-app.jpg}
...
An Android companion application serves as the primary
patient/clinician interface (Fig.~\ref{fig:androidapp}). The
application subscribes to the per-vital HiveMQ topics of
Fig.~\ref{fig:hivemq-arch} and presents real-time vital-sign cards
for SpO\textsubscript{2}, heart rate, and ambient temperature,
together with a rolling 30-minute history chart. Threshold-based
colour-coded alerts are triggered when SpO\textsubscript{2} drops
below 90\,\%, heart rate exceeds 100\,bpm, or ambient temperature
exceeds 38\,$^\circ$C. During testing, the broker's web console
confirmed three active topic subscriptions corresponding to the
SpO\textsubscript{2}, heart-rate, and temperature channels.
Multi-patient profile switching is retained as a planned extension
...
\item Author contribution: All authors contributed to the system
