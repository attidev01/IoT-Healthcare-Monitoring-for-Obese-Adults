...
(iv)~portable ECG integration (AD8232 module); (v)~non-invasive
glucose monitoring; (vi)~predictive machine-learning models trained
on longitudinal vital-sign data to forecast patient-specific
health-risk trajectories, extending the current threshold-based
anomaly detection; (vii)~additional physiological and environmental
sensors (e.g., accelerometer-based activity/fall detection and
ambient humidity) to broaden the monitored parameter set;
(viii)~migration from the current breadboard prototype to a custom
printed-circuit board, reducing per-unit cost and power consumption
for a battery-optimised, mass-producible device; and (ix)~a voice
interface.
